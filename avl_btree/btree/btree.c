...
void excluiBtree(ArvoreB *arvore){
    if(arvore->raiz){
        printf("Excluindo raiz\n");
        excluiNo(arvore->raiz, arvore->ordem);
    }
    printf("Excluindo Arvore\n");
    free(arvore);
}

void excluiNo(No *no, int ordem){
    int max = ordem *2;
    if(no){
        for(int i =0; i<max+2; i++){
            excluiNo(no->filhos[i], ordem);
            free(no->filhos[i]);
        }
        free(no->filhos);
        free(no->chaves);
...
int pesquisaBinaria(ArvoreB *arvore, No *no, int chave)
{
...
void adicionaChaveNo(ArvoreB *arvore, No *no, No *novo, int chave)
{
    int i = pesquisaBinaria(arvore, no, chave);
